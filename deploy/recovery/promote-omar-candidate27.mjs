// Promote Candidate27's three owned Tutorial Studio processes on Omar.
// The independent tutorial-uploader-exchange is never controlled here.
import fs from "node:fs";
import { spawnSync } from "node:child_process";
const current = "/opt/tutorial-review-omar-20260910-final-c26";
const target = "/opt/tutorial-review-omar-20260910-final-c27";
const archive = "/opt/runtime-app-candidate27.tgz";
const expectedArchiveSha256 = "1ea9ec8f7b798d241f335e1147b667f6dd9bd2791510a16ef77fbd4c7e8213f8";
...
if (!fs.statSync(current).isDirectory() || !fs.statSync(archive).isFile()) throw new Error("Candidate26 runtime or Candidate27 archive missing");
const archiveSha256 = run("sha256sum", [archive]).trim().split(/\s+/)[0];
if (archiveSha256 !== expectedArchiveSha256) throw new Error("Candidate27 archive checksum mismatch");
...
try {
  ...
  if (!healthy(port)) throw new Error("Candidate27 health gate failed");
  ...
  if (!healthy(port)) throw new AggregateError([error], "Candidate27 failed and Candidate26 rollback health failed");
