// Promote Candidate 25's three Tutorial Studio processes. The separately
...
const target = "/opt/tutorial-review-omar-20260910-final-c25";
const archive = "/opt/runtime-app-candidate25.tgz";
const expectedArchiveSha256 = "18cbe55d4264f884a486fa5cd473911722cfdd70a55a9903b24919b3f90b6087";
...
if (!fs.statSync(current).isDirectory() || !fs.statSync(archive).isFile()) {
  throw new Error("Candidate 21 or Candidate 25 archive is missing");
}
const archiveSha256 = run("sha256sum", [archive]).trim().split(/\s+/)[0];
if (archiveSha256 !== expectedArchiveSha256) throw new Error("Candidate 25 archive checksum mismatch");
const targetExists = fs.existsSync(target);
if (targetExists && !fs.statSync(`${target}/app`).isDirectory()) {
  throw new Error("Candidate 25 target is not an extracted retry target");
}
...
if (controlledNames.join(",") !== "tutorial-drive-uploader,tutorial-web,tutorial-worker") throw new Error("Unexpected Candidate 25 process set");
...
for (const [name, value] of [["live.json", live], ["drive-live.json", drive]]) {
  ...
  if (fs.existsSync(path)) {
    if (fs.readFileSync(path, "utf8") !== wanted) throw new Error(`Candidate 25 ${name} differs from the verified retry configuration`);
  } else {
...
try {
  ...
  if (!healthy) throw new Error("Candidate 25 health check did not become ready");
  ...
  if (selected.length !== 3 || selected.some((process) => process.pm2_env?.status !== "online" || (process.pm2_env?.pm_cwd !== `${target}/app` && !String(process.pm2_env?.pm_cwd).startsWith(`${target}/app/`)))) {
    throw new Error("Candidate 25 process cwd verification failed");
  }
  ...
  if (!rollbackHealthy || rollbackApps.length !== 3 || rollbackApps.some((process) => process.pm2_env?.status !== "online" || !String(process.pm2_env?.pm_cwd).startsWith(`${current}/app`)) || rolledBack.find((process) => process.name === "tutorial-uploader-exchange")?.pid !== exchangePid) {
    throw new AggregateError([error], "Candidate 25 promotion failed and automatic rollback could not be verified");
  }
  run("pm2", ["save"]);
  throw error;
}
