// Promote Candidate29's three owned Tutorial Studio processes on Omar.
// The independent tutorial-uploader-exchange is never controlled here.
import fs from "node:fs";
import { spawnSync } from "node:child_process";
const current = "/opt/tutorial-review-omar-20260910-final-c28";
const target = "/opt/tutorial-review-omar-20260910-final-c29";
const archive = "/opt/runtime-app-candidate29.tgz";
const expectedArchiveSha256 = "90a1e64770618fb7c06199d7c6358ad6008d84678f13eba818691e01b53e3fb4";
...
if (!fs.statSync(current).isDirectory() || !fs.statSync(archive).isFile()) throw new Error("Candidate28 runtime or Candidate29 archive missing");
const archiveSha256 = run("sha256sum", [archive]).trim().split(/\s+/)[0];
if (archiveSha256 !== expectedArchiveSha256) throw new Error("Candidate29 archive checksum mismatch");
...
try {
  ...
  if (!healthy(port)) throw new Error("Candidate29 health gate failed");
  ...
  if (!healthy(port)) throw new AggregateError([error], "Candidate29 failed and Candidate28 rollback health failed");
