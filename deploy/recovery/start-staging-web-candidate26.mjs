// Upgrade only the reviewed private VPS2 web canary to Candidate 26.
// Workers, writers, AI, Drive, upload, publication, and cleanup stay disabled.
import fs from "node:fs";
import { spawnSync } from "node:child_process";

const root = "/opt/tutorial-recovery-staging";
const image = "tutorial-recovery:20260910-candidate26";
const expectedImageId = "sha256:a15f34f762ac137672b11709d3d6697acacfd344cc2b1c74936ba37132f7b90e";
const previousImage = "tutorial-recovery:20260910-candidate25";
const mode = process.argv[2];
const composePath = process.argv[3];
if (!["--prepare-config", "--check", "--upgrade-verified-web"].includes(mode) || !composePath?.startsWith(`${root}/`) || !/^[\w/.-]+$/.test(composePath)) throw new Error("Explicit mode and verified staging compose path required");

const directory = fs.lstatSync(root);
if (!directory.isDirectory() || directory.isSymbolicLink() || directory.uid !== 0 || (directory.mode & 0o077)) throw new Error("Unsafe staging directory");
for (const path of [composePath, `${root}/.env`, `${root}/runtime.ai-20260909.env.candidate`]) {
  const item = fs.lstatSync(path);
  if (!item.isFile() || item.isSymbolicLink() || item.uid !== 0 || (item.mode & 0o077)) throw new Error("Unsafe staging configuration");
}
const run = (args) => {
  const result = spawnSync("docker", args, { encoding: "utf8", maxBuffer: 8 * 1024 * 1024, env: { ...process.env, STUDIO_IMAGE: image } });
  if (result.status !== 0) throw new Error("Staging Docker command failed; inspect privately");
...
if (!existing || existing.split("\n").length !== 1) throw new Error("Expected exactly one reviewed staging web container");
const current = JSON.parse(run(["inspect", existing]))[0];
if (![image, previousImage].includes(current.Config.Image)) throw new Error("Do not replace an unreviewed staging image");
const bindings = current.HostConfig.PortBindings;
if (Object.keys(bindings).join(",") !== "3000/tcp" || bindings["3000/tcp"].length !== 1 || bindings["3000/tcp"][0].HostIp !== "127.0.0.1" || bindings["3000/tcp"][0].HostPort !== "3118") throw new Error("Staging web is not the expected private target");

const overridePath = `${root}/candidate26.web-media.override.json`;
...
if (fs.existsSync(overridePath)) {
  ...
  if (!item.isFile() || item.isSymbolicLink() || item.uid !== 0 || (item.mode & 0o077) || fs.readFileSync(overridePath, "utf8") !== override) throw new Error("Conflicting override file");
} else if (mode === "--prepare-config") fs.writeFileSync(overridePath, override, { flag: "wx", mode: 0o600 });
else throw new Error("Candidate 26 override is absent; prepare it explicitly");
...
if (Object.keys(web.networks).sort().join(",") !== "default,web_access" || resolved.networks.web_access.internal) throw new Error("Unexpected web network posture");
for (const service of ["postgres", "redis"]) if (Object.keys(resolved.services[service].networks).join(",") !== "default") throw new Error("Database and queue must stay isolated");
if (web.ports.length !== 1 || web.ports[0].host_ip !== "127.0.0.1" || String(web.ports[0].published) !== "3118" || web.ports[0].target !== 3000) throw new Error("Staging port must remain loopback-only");
...
if (web.environment.LOCAL_MEDIA_ROOT !== "/opt/content-forge/media" || !web.volumes.some((volume) => volume.type === "bind" && volume.source === `${root}/media` && volume.target === "/opt/content-forge/media" && !volume.read_only)) throw new Error("Imported media mapping changed");
if (String(web.environment.VEOFORGE_IMAGES_ENABLED) !== "0") throw new Error("Live images must stay disabled");
