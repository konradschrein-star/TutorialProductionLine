...
readonly rehearsal="tutorial_studio_c26_rehearsal"
readonly stamp="$(date -u +%Y%m%dT%H%M%SZ)"
readonly backup_dir="/var/backups/tutorial-omar-pre-candidate26-${stamp}"
...
rehearsal_created=0
cleanup() {
  if [[ "$rehearsal_created" == "1" ]]; then runuser -u postgres -- dropdb --if-exists "$rehearsal" || true; fi
}
trap cleanup EXIT
