...
5. ✅ **DEPLOYED to the associate's VPS** `212.132.103.168` (Ubuntu 26.04): pnpm+Postgres 18+Redis provisioned; source at `/opt/tutorial-studio`; build green; `db:push`+seed; PM2 runs web+worker+drive-uploader (fork mode, `node --import tsx` web, ecosystem injects `.env`), boot-persistent; nginx proxies port 80 → :3000. **LIVE + login-verified at http://212.132.103.168** (studio renders, all lanes incl. translate registered, drive-uploader idles until Drive configured). REMAINING: friend's API keys (Settings → Credentials); change the default admin password; DNS cutover (deferred by owner); still to copy over: **his Keyword Tool clone** + later the uploader; purge `vps_deploy_key` from git history + rotate.
