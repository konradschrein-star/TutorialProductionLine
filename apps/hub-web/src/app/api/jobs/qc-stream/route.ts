...
/**
 * GET /api/jobs/qc-stream?job_id=...
 *
 * Server-Sent Events stream for real-time QC job updates.
 * Sends updates every 2 seconds while the current job is at AWAITING_QC.
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check permissions
    if (!hasPermission(session, "review:qc")) {
      ...
      async start(controller) {
        ...
        const pollInterval = setInterval(async () => {
          try {
            ...
            // If job left AWAITING_QC state, close the stream
            if (currentJob.status !== "AWAITING_QC") {
              clearInterval(pollInterval);
              controller.close();
              return;
            }

            // Send QC queue statistics
            ...
            controller.enqueue(
              encoder.encode(
                `event: qc-stats\ndata: ${JSON.stringify({
                  awaitingQCCount: awaitingCount,
                  errorCount: errorCount,
                })}\n\n`,
              ),
            );
          } catch (error) {
            console.error("Error in QC stream poll:", error);
    ...
    console.error("GET /api/jobs/qc-stream error:", error);
    return NextResponse.json(
      {
