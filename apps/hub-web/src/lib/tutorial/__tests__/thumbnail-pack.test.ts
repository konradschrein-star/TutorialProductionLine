...
describe("thumbnail publication pack", () => {
  it("uses exactly the five automatic translation languages", () => {
    expect(THUMBNAIL_PACK_LANGUAGES).toHaveLength(5);
  ...
  it("does not accept fewer than five variants", () => {
    ...
    expect(result.variants[4]?.reasons).toContain("translation job missing");
  });
