#!/usr/bin/env tsx
/**
 * Seed: Casually Explained Editorial (Script Provided) Template
 *
 * Casually Explained style with placeholder boxes format for users who provide their own scripts.
...
async function seedTemplate() {
  console.log("[seed-casually-explained-editorial-script-provided] Loading config...");
  ...
  const templateName = "Casually Explained Editorial (Script Provided)";
  ...
  const templateData = {
    name: templateName,
    description: "Casually Explained style with placeholder boxes for pre-written scripts. User provides the script, system generates images with Ken Burns effects, TTS narration, and word-aligned captions. Faster workflow for creators who prefer to write their own content.",
...
Output format: keyword1, keyword2, keyword3`,
  ...
  if (existing) {
    ...
    console.log(`[seed-casually-explained-editorial-script-provided] Updated existing template: ${existing.id}`);
  } else {
    const [inserted] = await db
      .insert(contentTemplates)
      .values(templateData)
      .returning({ id: contentTemplates.id });
    console.log(`[seed-casually-explained-editorial-script-provided] Inserted new template: ${inserted.id}`);
  }

  console.log("[seed-casually-explained-editorial-script-provided] Done.");
  await sql.end();
  process.exit(0);
}

seedTemplate().catch((err) => {
  console.error("[seed-casually-explained-editorial-script-provided] Fatal:", err);
  process.exit(1);
});
