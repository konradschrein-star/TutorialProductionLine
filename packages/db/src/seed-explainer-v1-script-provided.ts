#!/usr/bin/env tsx
/**
 * Seed: Explainer V1 (Script Provided) Template
 *
...
async function seedTemplate() {
  console.log("[seed-explainer-v1-script-provided] Loading config...");
  ...
  const templateName = "Explainer V1 (Script Provided)";
  ...
  const templateData = {
    name: templateName,
    description: "Educational explainer for pre-written scripts. User provides the script, system generates images with Ken Burns effects, TTS narration, and word-aligned captions. Faster workflow for creators who prefer to write their own content.",
...
Output format: keyword1, keyword2, keyword3`,
  ...
  if (existing) {
    ...
    console.log(`[seed-explainer-v1-script-provided] Updated existing template: ${existing.id}`);
  } else {
    const [inserted] = await db
      .insert(contentTemplates)
      .values(templateData)
      .returning({ id: contentTemplates.id });
    console.log(`[seed-explainer-v1-script-provided] Inserted new template: ${inserted.id}`);
  }

  console.log("[seed-explainer-v1-script-provided] Done.");
  await sql.end();
  process.exit(0);
}

seedTemplate().catch((err) => {
  console.error("[seed-explainer-v1-script-provided] Fatal:", err);
  process.exit(1);
});
