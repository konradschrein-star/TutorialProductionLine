...
export const channels = pgTable(
  "channels",
  {
    ...
    // Explicit destination understood by the separate uploader service.
    // NULL means this Studio channel is intentionally not dispatchable. Never
    // derive this from a YouTube id, handle, display name, or language: those
    // values do not identify the uploader's isolated browser profile.
    uploader_channel_key: varchar("uploader_channel_key", { length: 64 }),
  ...
  (table) => ({
    uploaderChannelKeyUnique: uniqueIndex(
      "channels_uploader_channel_key_unique",
    )
      .on(table.uploader_channel_key)
      .where(sql`${table.uploader_channel_key} IS NOT NULL`),
    uploaderChannelKeyFormat: check(
      "channels_uploader_channel_key_format",
      sql`${table.uploader_channel_key} IS NULL OR ${table.uploader_channel_key} ~ '^[a-z][a-z0-9_-]{0,63}$'`,
    ),
  }),
);
