#!/usr/bin/env tsx
/**
 * Seed: Casually Explained Automated (Script Provided) Template
 *
 * Casually Explained style hybrid composition format for users who provide their own scripts.
...
async function seedTemplate() {
  console.log(
    "[seed-casually-explained-automated-script-provided] Loading config...",
  );
  try {
    loadConfig();
  } catch (e) {
  ...
  const templateName = "Casually Explained Automated (Script Provided)";
  ...
  const templateData = {
    name: templateName,
    description:
      "Casually Explained style hybrid composition for pre-written scripts. User provides the script, system generates images with Ken Burns effects, TTS narration, and word-aligned captions. Faster workflow for creators who prefer to write their own content.",
...
Output format: keyword1, keyword2, keyword3`,
    ...
    render_config: {
      ...
      settings: {
        ...
        width: 1920,
        height: 1080,
        video_codec: "h264_nvenc", // GPU encoding (fallback to libx264)
        audio_codec: "aac",
        video_crf: 20,
        video_preset: "fast",
      },
      ...
      ken_burns: {
        enabled: true,
        zoom_range: [1.04, 1.08], // 4-8% zoom per scene
        pan_intensity: 0.2, // 0-1, amount of lateral movement
        easing: "smoothstep" as const, // smoothstep | ease_in_out | linear
      },
    },
    ...
    metadata: {
      ...
      style_preset: {
        ...
        prompt_suffix:
          "professional educational aesthetic, 16:9 aspect ratio, high quality stock photo style",
        caption_style: {
  ...
  if (existing) {
    ...
    console.log(
      `[seed-casually-explained-automated-script-provided] Updated existing template: ${existing.id}`,
    );
  } else {
    ...
    console.log(
      `[seed-casually-explained-automated-script-provided] Inserted new template: ${inserted.id}`,
    );
  }

  console.log("[seed-casually-explained-automated-script-provided] Done.");
  await sql.end();
  process.exit(0);
}
...
seedTemplate().catch((err) => {
  console.error(
    "[seed-casually-explained-automated-script-provided] Fatal:",
    err,
  );
  process.exit(1);
});
